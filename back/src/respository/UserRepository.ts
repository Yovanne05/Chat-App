...
export class UserRepository {
    ...
    static async addFriend(idUser: string, idNewFriend: string): Promise<boolean> {
        try {
    ...
    static async removeFriend(idUser: string, idFriend: string): Promise<boolean> {
        try {
            ...
            user.friends = user.friends.filter((f: Types.ObjectId) => f.toString() !== idFriend);
            friend.friends = friend.friends.filter((f: Types.ObjectId) => f.toString() !== idUser);

            await user.save();
            await friend.save();

            return user.friends.length !== initialUserLength || friend.friends.length !== initialFriendLength;
        } catch (error) {
